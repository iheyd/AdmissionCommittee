...
{
    public partial class Rating1UserControl : UserControl
    {
        public Rating1UserControl()
        {
            InitializeComponent();
            dataGridViewRating1.CellClick += DataGridView_CellClick;
            dataGridViewRating1.RowHeadersVisible = false;
            dataGridViewRating1.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            dataGridViewRating1.CellFormatting += DataGridView_CellFormatting;
            LoadRatingData1();
        }

        public void LoadRatingData1()
        {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                try
                {
                    ...
                    {
                        cmd.Parameters.AddWithValue("directionId", 1);
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            ...
                            dataGridViewRating1.DataSource = displayTable;

                            if (dataGridViewRating1.Columns.Contains("Базовый ID"))
                                dataGridViewRating1.Columns["Базовый ID"].Visible = false;
                        }
                    }
                }
                catch (Exception ex)
